#!/usr/bin/env bash
# Headless: server + client; client world-interacts first room_label / corridor / special_feature / area / building (Phase 5.5).
set -eu
...
godot4 --headless --path . --client --ip 127.0.0.1 --headless --port "$PORT" --no-fog --smoke-labels-probe Main.tscn 2>&1 | tee "$TMP/cli.log"
CLI=$?
wait "$SRV" || true
...
if ! grep -qE 'world_interaction remote peer_id=.* kind=(room_label|corridor_label|special_feature|area_label|building_label) ' "$TMP/srv.log"; then
	echo "ERROR: expected server log world_interaction remote … kind=label/feature ($TMP/srv.log)" >&2
	exit 1
fi

echo "smoke_labels: OK (port=$PORT)"
rm -rf "$TMP"
exit 0
