#!/usr/bin/env bash
# Headless: server + client; client sends encounter Fight on first encounter cell (Phase 6).
set -eu
...
TMP="${TMPDIR:-/tmp}/dng_smoke_combat_$$"
mkdir -p "$TMP"
SEED=42
PORT="$(pick_free_port)"
...
godot4 --headless --path . --client --ip 127.0.0.1 --headless --port "$PORT" --no-fog --smoke-combat-probe Main.tscn 2>&1 | tee "$TMP/cli.log"
CLI=$?
wait "$SRV" || true
...
if ! grep -qF "encounter_fight_resolved peer_id=" "$TMP/srv.log"; then
	echo "ERROR: expected server log encounter_fight_resolved ($TMP/srv.log)" >&2
	exit 1
fi

echo "smoke_combat: OK (port=$PORT)"
rm -rf "$TMP"
exit 0
