#!/usr/bin/env bash
# Headless: server + client; client finds a plain `treasure` cell, world-interact + treasure dismiss.
set -eu
...
godot4 --headless --path . --client --ip 127.0.0.1 --headless --port "$PORT" --no-fog --smoke-treasure-probe Main.tscn 2>&1 | tee "$TMP/cli.log"
CLI=$?
wait "$SRV" || true
...
if ! grep -qF "treasure_dismiss peer_id=" "$TMP/srv.log"; then
	echo "ERROR: expected server log 'treasure_dismiss peer_id=' ($TMP/srv.log)" >&2
	exit 1
fi

echo "smoke_treasure: OK (port=$PORT)"
rm -rf "$TMP"
exit 0
