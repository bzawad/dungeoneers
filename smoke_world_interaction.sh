#!/usr/bin/env bash
# Headless: server + client with --smoke-world-interaction-probe (--no-fog so any encounter cell is valid).
set -eu
...
godot4 --headless --path . --client --ip 127.0.0.1 --headless --port "$PORT" --no-fog --smoke-world-interaction-probe Main.tscn 2>&1 | tee "$TMP/cli.log"
CLI=$?
wait "$SRV" || true
...
if ! grep -qF "world_interaction remote" "$TMP/srv.log"; then
	echo "ERROR: expected server log 'world_interaction remote' ($TMP/srv.log)" >&2
	exit 1
fi

echo "smoke_world_interaction: OK (port=$PORT)"
rm -rf "$TMP"
exit 0
