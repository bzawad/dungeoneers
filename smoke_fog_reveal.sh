#!/usr/bin/env bash
# Headless: server + client with --smoke-fog-reveal-probe (manual fog cell reveal vs Explorer).
set -eu
...
TMP="${TMPDIR:-/tmp}/dng_smoke_fog_reveal_$$"
mkdir -p "$TMP"
SEED=90909
PORT="$(pick_free_port)"
...
godot4 --headless --path . --client --ip 127.0.0.1 --headless --port "$PORT" --smoke-fog-reveal-probe Main.tscn 2>&1 | tee "$TMP/cli.log"
CLI=$?
wait "$SRV" || true
...
if ! grep -qE "fog_reveal_probe ok|fog_reveal_probe skipped" "$TMP/cli.log"; then
	echo "ERROR: expected fog_reveal_probe ok or skipped in client log ($TMP/cli.log)" >&2
	exit 1
fi

echo "smoke_fog_reveal: OK (port=$PORT)"
rm -rf "$TMP"
exit 0
