...
if ! grep -qE "encounter_evade_(success|failed) peer_id=" "$TMP/srv.log"; then
	echo "ERROR: expected server log encounter_evade_success or encounter_evade_failed ($TMP/srv.log)" >&2
	exit 1
fi
